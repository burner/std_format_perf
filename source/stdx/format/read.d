...
Source: $(PHOBOSSRC stdx.format/read.d)
 */
module stdx.format.read;

private template WideElementType(T)
{
...
import stdx.format.internal.read;
...
Params:
    input = the $(REF_ALTTEXT input range, isInputRange, std, range, primitives),
            to read from
    spec = a $(MREF_ALTTEXT format string, stdx.format)
    T = type to return
