...
/*
   ...
   Source: $(PHOBOSSRC stdx.format/internal/write.d)
 */
module stdx.format.internal.write;

import stdx.format.spec : FormatSpec;
import std.range.primitives : isInputRange;
import std.traits;

private void assertCTFEable(alias dg)()
{
...
@safe pure unittest
{
    ...
    import stdx.format : FormatException;
...
if (is(FloatingPointTypeOf!T) && !is(T == enum) && !hasToString!(T, Char))
{
    ...
    import stdx.format.internal.floats : printFloat;
...
@safe unittest
{
    ...
    import stdx.format : formatValue;
...
if (isInputRange!T)
{
    ...
    import stdx.format : FormatException, formatValue, NoOpSink;
...
{
    import stdx.format : formattedWrite;
    import std.range.primitives : put;
...
{
    import stdx.format : enforceFmt, formatValue;
    import std.range.primitives : put;
...
@safe unittest
{
    ...
    s = format("%s", c2);
    assert(s == "immutable(stdx.format.internal.write.C)");

    const(C) c3 = new C();
    s = format("%s", c3);
    assert(s == "const(stdx.format.internal.write.C)");

    shared(C) c4 = new C();
    s = format("%s", c4);
    assert(s == "shared(stdx.format.internal.write.C)");
}
...
void enforceValidFormatSpec(T, Char)(scope const ref FormatSpec!Char f)
{
    ...
    import stdx.format.internal.write : hasToString, HasToStringResult;
...
{
    import std.array : appender;
    import stdx.format.write : formattedWrite, formatValue;
    import std.range.primitives : put;
...
{
    import std.array : appender;
    import stdx.format.spec : singleSpec;
...
{
    import std.range.primitives : put;
    import stdx.format.write : formatValue;
...
private void formatTest(T)(string fmt, T val, string[] expected, size_t ln = __LINE__, string fn = __FILE__) @safe
{
    ...
    import std.conv : text;
    import std.exception : enforce;
    import stdx.format.write : formattedWrite;
